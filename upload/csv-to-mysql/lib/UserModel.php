...
class UserModel
{
    ...
    function getAllUser()
    {
        $sqlSelect = "SELECT * FROM contoh_import";
    ...
    {
        $sql = "SELECT userName FROM contoh_import WHERE userName = ?";
        $paramType = "s";
        $paramArray = array(
            $userName
        );
        ...
        if (empty($result)) {
            ...
            $sql = "INSERT into contoh_import (userName,password,firstName,lastName)
                       values (?,?,?,?)";
            $paramType = "ssss";
            $paramArray = array(
                $userName,
